...
VCFDataToSaveOther<-subset(VCFSepFilterApplied,
                           VCFSepFilterApplied$ACm>1)[,c(1:7,17,11,12)]
##########################
#Save plots
...
SingletonsOutfilename<-paste(gsub(".vcf","",args[1]),"_singletons_",FixedCutOff,".vcf",
                               sep="")
OtherOutfilename<-paste(gsub(".vcf","",args[1]),"_nonsingletons_",FixedCutOff,".vcf",
                             sep="")
...
write.table_with_header(VCFDataToSaveSingletons,
                        file=SingletonsOutfilename, header)
write.table_with_header(VCFDataToSaveOther,
                        file=OtherOutfilename, header)

#############################
####Select variants in introns
VCFOnlyMissense<-subset(VCFDataToSaveSingletons,
                        grepl("missense",VCFDataToSaveSingletons$ANNAll))
VCFOnlySynonymous<-subset(VCFDataToSaveSingletons,
                        grepl("synonymous",VCFDataToSaveSingletons$ANNAll))
VCFOnlyIntrons<-subset(VCFDataToSaveSingletons,
                       grepl("intron",VCFDataToSaveSingletons$ANNAll) &&
                         !(VCFDataToSaveSingletons$V2 %in% c(VCFOnlyMissense,VCFOnlySynonymous)))

singlNonGeneFileName<-paste(gsub(".vcf","",args[1]),"_singletonsMissense_",FixedCutOff,".vcf",
                            sep="")
write.table_with_header(VCFOnlyMissense,
                        file=singlNonGeneFileName, header)